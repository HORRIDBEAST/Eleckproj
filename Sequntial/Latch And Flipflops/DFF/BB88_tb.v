...
module dff_PE_SRH_tb;
    reg d, clk, rst;
    wire q;
    
    // Instantiate the D Flip-Flop module
    dff_PE_SRH uut (.q(q), .d(d), .clk(clk), .rst(rst));
    
    // Clock generation (10ns period -> 5ns high, 5ns low)
    always #5 clk = ~clk; 

    initial begin
        $dumpfile("dff_PE_SRH.vcd");
        $dumpvars(0, dff_PE_SRH_tb);
        
        $monitor("Time=%0t | d=%b, clk=%b, rst=%b -> q=%b", $time, d, clk, rst, q);
        
        // Initialize values
        clk = 0; d = 0; rst = 0;  // Start with reset deactivated
        ...
        #10 rst = 1;  // Set reset high (should take effect on next posedge clk)
        ...
        #10 rst = 1; d = 1;  // Reset while d=1 (should reset on next clk posedge)
        #10 rst = 0; d = 0;  // Release reset
